...
#pragma aux rd_kb = \
"       int  0x16              " \
parm [ax]  [cx] modify [ax];

 Bit16u  rd_kb(Bit16u par1, Bit16u par2);
...
static void init_comport(void)
{
        ...
        //=========================================
        outb(UART_TR, 1);    // set up uart for 115.2kbps
//        outb(UART_TR, 3);          // set up uart for 38.4 kbps
//        outb(UART_TR, 12);        // set up uart for 9.6 kbps
        //=========================================
        outb(UART_IE,  0x00);     //=====================         
...
//------------------------------------------------------------------------------------------------------
static void send (Bit16u action, Bit8u  c)  /////////////////////             ORIGINAL
{
...
static void send_my (Bit16u action, Bit8u  c) //////////////////////// 2013.06.16
{
    if(action & BIOS_PRINTF_SCREEN) {
        // if(c == '\n') biosfn_prnt_char('\r');
        biosfn_prnt_char(c);
...
void CLS (void)
{
  ...
  Bit16u N;
    ...
    addr  =0x0;
    for (N=0; N<2000; N++) {
        write_byte( 0xB800, addr , 0x0); //CHAR
        addr++;
        write_byte( 0xB800, addr , 0x7); //ATTR
        addr++;
   }
...
void __cdecl int2_NMI_function(void)
{
Bit16u seg;
Bit8u val;
Bit16u flash_data ;
      ...
      // write_byte(ebda_seg, 0x0027, 0x83);                     // Set packet size and driver

       //outb(UART,0x33);
/*
            val = read_word(0x0000, 0x046c);
            bios_printf(BIOS_PRINTF_SCREEN,    "NMI: 0x046c : %x ; ", val);
            val = read_word(0x0000, 0x046e);
            bios_printf(BIOS_PRINTF_SCREEN,    " 0x046e: %x ; ", val);
            val = read_word(0x0000, 0x0470);
            bios_printf(BIOS_PRINTF_SCREEN,    " 0x0470 : %x  \n", val);
*/
       
           // seg = 0x07C0;  // Start UP 
           seg = 0xC000 ;     //  VGA BIOS
           bios_printf(BIOS_PRINTF_COMPORT,    "NMI: ADDRR %x : \n ", seg);
           val = read_byte(seg, 0x0000);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0001);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0002);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0003);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0004);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0005);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0006);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0007);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : \n ", val);

           val = read_byte(seg, 0x0008);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x0009);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000a);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000b);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000c);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000d);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000e);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : ", val);
           val = read_byte(seg, 0x000f);
           bios_printf(BIOS_PRINTF_COMPORT,    " : %x : \n ", val);


           /*
           outw(0x238+2, 0x0000); // = Hi ADDR
           outw(0x238,     0x8000); // = Lo ADDR
           outw(0x238,     0x8000); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           //
           outw(0x238,     0x8001); // = Lo ADDR
           outw(0x238,     0x8001); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           //
           outw(0x238,     0x8002); // = Lo ADDR
           outw(0x238,     0x8002); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           //
           outw(0x238,     0x8003); // = Lo ADDR
           outw(0x238,     0x8003); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           //
           outw(0x238,     0x8004); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           outw(0x238,     0x8005); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
            outw(0x238,     0x8006); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
           outw(0x238,     0x8007); // = Lo ADDR
           flash_data  = inw(0x238);
           bios_printf(BIOS_PRINTF_SCREEN,    " : %x : ", flash_data);
...
static Bit16u get_kb_char(void)  
{
Bit8u  zflag;
Bit16u  keyb_char;
        zflag = 1;                           // Устанавливаем флаг, который будет сброшен при нажатии на
                                             // любую клавишу                                      
        __asm {
nokey: 
                mov   ax, 0100h
                int   16h                    // Если нажатия не было, 
                jz    nokey                  // продолжаем выполнение программы
                mov   zflag, 0               // При нажатии на любую клавишу сбрасываем флаг
                }
       keyb_char = rd_kb(0x0, 0x0);          // Если флаг сброшен, читаем код нажатой клавиши из буфера
       return (keyb_char);
}
...
void __cdecl print_bios_banner(void)
{
   ...
   //
    ...
    bios_printf(BIOS_PRINTF_COMPORT, "ZET COM 115200  \n");
    //====================================== 
    //CLS(); //- CLEAR SCREEN;
    ...
    mem_test (0x0000,                     0x0000,   0xFFFE, 0x0,        0x1);
    mem_test (0x1000,                   0x0000, 0xFFFE, 0x0,        0x1);
    //////////// =====================VGA
    mem_test (0xA000;                   0x0000, 0xFFFE, 0x0,        0x1);
    mem_test (0xB000;                   0x0000, 0xFFFE, 0x0,        0x1);
    ///////////  ===============================================
    */
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
}

void __cdecl mainZ (void){
Bit16u   kb_char;
    write_byte(0xB800, 80*24*2, 0x53);  // =S
    while(1)
    {
                 kb_char = get_kb_char();
                 bios_printf(BIOS_PRINTF_SCREEN, " Keyboard Pess %x  \r" , kb_char);
    }
}
...
void __cdecl init_boot_vectors(void)
{
    ...
    if(sd_error) {
        ...
        bios_printf(BIOS_PRINTF_SCREEN,"SD card controller was initialized \n");
        switches = inb(0xf100);
...
void __cdecl int09_function(Bit16u rAX)
{
    ...
    switch(scancode) {
        ...
        default:
            ...
            if(scancode > MAX_SCAN_CODE) {  //#define MAX_SCAN_CODE        0x58
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// INT 13
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////====================== Diskette service function ===================================
//================================================================================
//  Transfer Sector drive 
#define FLASH_FLOPPY   0x0100000         // Starting address of floppy on flash 
...
static void transf_sect_drive_a(Bit16u Sector, Bit16u s_segment, Bit16u s_offset)
{
   ...
   sectloop:    mov   ax, bx             // Put bx into ax
                ...
                out   dx, ax             // Save LSB address word  ///////////////////////////////////////////////////////////// FIX BUG!!!
...
Bit16u rDS, rES, rDI, rSI, rBP, rBX, rDX, rCX, rAX, rIP, rCS, rFLAGS;
{
    ...
    if(!(drive_map & (1<<(GET_DL()&0x7f)))) {    // allow 0, 1, or 2 disks
        ...
        //printf("int13_harddisk_c DRIVE MAP: GET_DL (0x%x)\n", GET_DL());
        //HLT(); //
        return;
    }

///////////////////////////////////////////////////////////////////////////////====================================================================================================
    //printf("int13_harddisk_c COMMAND: (0x%x)\n", GET_AH());
    //HLT(); //- PASS
    ...
    switch(GET_AH()) {      // AH = Disk command
        ...
        case 0x02:                              // read disk sectors
           ...
           //printf("int13_harddisk_c READ cylinder: (0x%x)\n", cylinder);                     // =  0x
           //printf("int13_harddisk_c READ sector:   (0x%x)\n", sector);                         // = 0x1
           //printf("int13_harddisk_c READ head:   (0x%x)\n", head);                              // =  0x0
           //printf("int13_harddisk_c READ num_sectors:   (0x%x)\n", num_sectors); // = 0x1
           //HLT(); //- PASS
            ...
            tempbx = rBX;
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            //printf("int13_harddisk_c READ log_sector: (0x%x)\n", log_sector);
            //printf("int13_harddisk_c To SEG:    (0x%x)\n", rES);
            //printf("int13_harddisk_c To ADDR: (0x%x)\n", rBX);   

            __asm { sti }  //;; enable higher priority interrupts
            while(1) {
                addr_l = ((Bit16u) log_sector) << 9;   //  -????
                addr_h =  (Bit16u) (log_sector >> 7); // Hi byte
            ...
            //printf("int13_harddisk_c log_sector was readed \n");  
            //HLT(); //
   ...
   // #endif
    ...
    switch(e.type) {
        ...
        case IPL_TYPE_HARDDISK: // HDD 
            ...
            //printf("Return from  int  0x13 - read sector bios  \n" );
...
void __cdecl int14_function(Bit16u rAX, Bit16u rDX, Bit16u rDS, Bit16u rIP, Bit16u rCS, Bit16u rFLAGS)
{
    ...
    if((rDX < 4) && (addr > 0)) {
        switch(GET_AH()) {
            case 0:                                                            //  BAUD  RATE   AL
                outb((addr + 3), inb(addr + 3) | 0x80);      //  UART_LC   LINE CONTROL: SET 7bit - DIV access
                if(GET_AL() & 0xE0 == 0) {                      
                    outb(addr, 0x17);                                    // SET 0x0417  = 1047 (for 1.8432Mhz :  110)
                    outb(addr+1, 0x04);
                }
                else { /////////////////////////////////////////// AL: (bit 7,6,5)  * 32 - BAUD RATE
                    val16 = 0x600 >> (((GET_AL()) & 0xE0) >> 5);
                    outb(addr, val16 & 0xFF);
                    outb((addr + 1), val16 >> 8);
                } ////////////////////////////////////////////////////  AL bit 4,3,2,1,0  - LINE CONTROL ////////////////////////////
                val8 = GET_AL() & 0x1F; outb((addr + 3), val8);   //LINE CONTROL: bit 7,6,5- clear
                val8 = inb(addr + 5); SET_AH(val8);                       // RETURN  AH- line status
                val8 = inb(addr + 6); SET_AL(val8);                       //                     AL - modem status 
                CLEAR_CF();                                                           // Clear Flag 
                break;
            case 1:                                                                           // send CHAR ============================>
                timer = read_word(0x0040, 0x006C);
                while(((inb(addr+5) & 0x60) != 0x60) && (counter)) {  // holding or transm reg empty OR counter =0
                ...
                if(counter > 0) {                                                        //- NORMAL OPERATION
                    outb(addr, GET_AL());                                        // send AL ==========>
                    val8 = inb(addr + 5); SET_AH(val8);                 //  read Status Line: AH
                }
                else {
                    SET_AH(0x80);                                                  //  overtime -  NOT READY
                }
                CLEAR_CF();
                break;
            case 2:                                                                         // receive CHAR <============================
                timer = read_word(0x0040, 0x006C);
                while(((inb(addr + 5) & 0x01) == 0) && (counter)) { // DATA READY reg empty OR counter =0
                    val16 = read_word(0x0040, 0x006C);
                    if(val16 != timer) {
                        timer = val16;
                        counter--;
                ...
                if(counter > 0) {                                                      //- NORMAL OPERATION
                    val8 = inb(addr + 5); SET_AH(val8);               //  read Status Line: AH
                    val8 = inb(addr    ); SET_AL(val8);                  //  AL - received CHAR
                }
                else {
                    SET_AH(0x80);                                                //  overtime -  NOT READY
                }
