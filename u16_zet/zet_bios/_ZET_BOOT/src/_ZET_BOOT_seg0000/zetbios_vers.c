...
Bit32u  do_go_exec (Bit32u (*entry)(void))
{
        return entry ();
}
...
"       push bx                            " \
"       mov  bp, sp                     " \
"       call  dword ptr 0x0[bp]  " \
parm [bx];

#pragma aux PUSH_STACK= \
"       push bx                              " \
parm [bx];
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#define BOOT_STRING   "ZET BOOT \n"    
void __cdecl print_bios_banner(void)
{
   Bit8u val, pattern, incr, readback;
   Bit16u addr, start, end, segment, error;
   Bit16u s_addr;
   //===========
   Bit16u NofChar; 
   //===========
    init_comport();
    bios_printf(BIOS_PRINTF_COMPORT, BOOT_STRING);
    bios_printf(BIOS_PRINTF_COMPORT, "ZET COM 115200 \n");
    //======================================
    CLS();
    bios_printf(BIOS_PRINTF_SCREEN, BOOT_STRING);
    //===================================================
    segment = 0x0000;
    pattern =0x0;
    incr =1;
    ...
    mem_test (segment, start, end, pattern, incr);
   //========================================
    segment = 0xF000;
    pattern =0x0;
    incr =1;
   ...
   /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    while(1)
    {
        bios_printf(BIOS_PRINTF_SCREEN,  "BOOT..... \n");
        bios_printf(BIOS_PRINTF_COMPORT,  "BOOT..... \n");
        //=======1 Read SEGMENT===============
        val = Read_UART();
        segment = Read_UART();
        segment =  (segment<<8) + val;
        CLS();
        bios_printf(BIOS_PRINTF_SCREEN,  "SEGMENT : %x \n", segment);
        bios_printf(BIOS_PRINTF_COMPORT,  "SEGMENT : %x \n", segment);

        //=======2 Read Load ADDRESS =========
        val = Read_UART();
        start = Read_UART();
        start =  (start<<8) + val;
        bios_printf(BIOS_PRINTF_SCREEN,  "Load ADDRESS : %x \n", start);
        bios_printf(BIOS_PRINTF_COMPORT,  "Load ADDRESS : %x \n", start);

        //=======3 Read Nof Byte =========
        val = Read_UART();
        NofChar = Read_UART();
        NofChar =  (NofChar<<8) + val;
        bios_printf(BIOS_PRINTF_SCREEN,  "Nof LoadByte : %x \n", NofChar);
        bios_printf(BIOS_PRINTF_COMPORT,  "Nof LoadByte : %x \n", NofChar);

        //=======4 Read Exec ADDR =========
        val = Read_UART();
        s_addr = Read_UART();
        s_addr =  (s_addr<<8) + val;
        bios_printf(BIOS_PRINTF_SCREEN,  "Exec ADDR : %x \n", s_addr);
        bios_printf(BIOS_PRINTF_COMPORT,  "Exec ADDR : %x \n", s_addr);
        ...
        { 
            val = Read_UART();
            //LED(val);
            //bios_printf(BIOS_PRINTF_SCREEN, "UART_CHAR : %x \n", val);
            //bios_printf(BIOS_PRINTF_SCREEN, "UART_ADDR : %x \n", addr);
            write_byte( segment, addr , val);
        }

        if  (!(s_addr == 0xFFFF))
        {
            bios_printf(BIOS_PRINTF_SCREEN, "GO : %x \n", s_addr);
            bios_printf(BIOS_PRINTF_COMPORT, "GO : %x \n", s_addr);
            //do_go_exec ((void *)s_addr);
            ////////////////////////////////////////////
            PUSH_STACK(segment );
            JUMP_C(s_addr);
            ///////////////////////////////////////////
...
static void mem_test (Bit16u tested_segment, Bit16u start, Bit16u end, Bit8u pattern, Bit8u incr)
{
   ...
   bios_printf(BIOS_PRINTF_SCREEN, "ERROR -NO \r");
   error = 0x0;
   //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
       ...
       for (addr=start,val=pattern; addr<end+1; addr++) {
           ...
           if (readback != val) {
                //================ERROR===================================================
                bios_printf(BIOS_PRINTF_SCREEN, "TEST MEM ERR: addr - %x, data- %x \r", addr, readback);
                error++;
               //==========================================================================
           }
           val += incr;
     ...
     bios_printf(BIOS_PRINTF_SCREEN, "FINISH, ERROR Num: %x \n", error);
}
